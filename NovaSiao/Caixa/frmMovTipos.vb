...
Public Class frmMovTipos
    ...
    Private _DadosOrigem As DadosOrigem
    '
...
#Region "NEW | PROPERTY"
    ...
    End Property
    '
    Public Property propDadosOrigem() As DadosOrigem
        Get
            Return _DadosOrigem
        End Get
        Set(ByVal value As DadosOrigem)
            _DadosOrigem = value
            '    
            Select Case value
    ...
    End Property
    '
    Sub New(myDadosOrigem As DadosOrigem, Optional procura As Boolean = False,
...
#Region "MENUSTRIP"
    ...
    Private Sub DesativarToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DesativarToolStripMenuItem.Click
        ...
        If IsDBNull(dgvListagem.CurrentRow.Cells(0).Value) Then Exit Sub
        '
        '--- verifica se existe alguma Forma de pagamento que usa esse Tipo
        Dim mBLL As New MovimentacaoBLL
        Dim Ver As Integer = mBLL.MovTipo_Atividade(dgvListagem.CurrentRow.Cells(0).Value)
        '
        If Ver > 0 Then
            MessageBox.Show("Não é possivel Desativar esse Tipo porque existe(m) " & Ver & " Forma(s) de Movimentação " &
                            "cadastradas com ele...", "Desativar Tipo", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            Exit Sub
        End If
        '
        '--- altera o valor
...
#Region "SALVAR REGISTRO"
    ...
    End Sub
    '
    ' SALVAR O REGISTRO MOVTIPO (INSERIDO | ALTERADO)
    Private Sub SalvarRegistro_Tipo()
        '
        ...
        For Each r As DataGridViewRow In dgvListagem.Rows
            ...
            If dtOrigem.Rows(r.Index).RowState = DataRowState.Modified Then ' registro ALTERADO
                '
                If Salva_MovTipo(r.Index, False) = True Then
                    regSalvos += 1
                End If
                '
            ElseIf dtOrigem.Rows(r.Index).RowState = DataRowState.Added Then ' registro NOVO
                '
                If Salva_MovTipo(r.Index, True) = True Then
                    regSalvos += 1
                End If
                '
            End If
        ...
        '--- mensagem de sucesso---
        If regSalvos > 0 Then
            MessageBox.Show("Sucesso ao salvar registro(s) de Tipo de Movimentação" & vbNewLine &
    ...
    End Sub
    '
    Private Function Salva_MovTipo(myRowIndex As Integer, Inserir As Boolean) As Boolean
        '
        Dim mBLL As New MovimentacaoBLL
